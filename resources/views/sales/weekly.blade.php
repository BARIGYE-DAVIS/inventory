...
@section('page-title')
    <i class="fas fa-calendar-week text-blue-600 mr-2"></i>Weekly Sales - Week {{ now()->weekOfYear }}
@endsection

@section('content')
<!-- Stats Card -->
<div class="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-xl shadow-lg p-6 mb-6">
    <div class="flex items-center justify-between">
        <div>
            <p class="text-blue-100 text-sm">Total Sales This Week</p>
            <p class="text-4xl font-bold mt-2">UGX {{ number_format($totalAmount, 0) }}</p>
            <p class="text-blue-100 text-sm mt-1">{{ $totalSales }} transactions • {{ now()->startOfWeek()->format('M d') }} - {{ now()->endOfWeek()->format('M d, Y') }}</p>
        </div>
...
<div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-6">
    ...
    @foreach($daysOfWeek as $index => $day)
        @php
            ...
            $dayData = $dailyBreakdown[$date->format('Y-m-d')] ?? ['sales' => 0, 'revenue' => 0];
            $isToday = $date->isToday();
        @endphp
        <div class="bg-white rounded-lg shadow p-4 {{ $isToday ? 'border-2 border-blue-500' : '' }}">
            <p class="text-xs text-gray-500 uppercase">{{ substr($day, 0, 3) }}</p>
            <p class="text-sm font-semibold text-gray-700">{{ $date->format('M d') }}</p>
            <p class="text-lg font-bold text-blue-600 mt-1">{{ $dayData['sales'] }}</p>
            <p class="text-xs text-gray-600">UGX {{ number_format($dayData['revenue'], 0) }}</p>
        </div>
    @endforeach
</div>
...
<div class="bg-white rounded-xl shadow-lg p-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 space-y-4 md:space-y-0">
        ...
        <div class="flex space-x-2 overflow-x-auto">
            ...
            <a href="{{ route('sales.weekly') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg whitespace-nowrap">
        ...
        <div class="flex space-x-2">
            ...
            <!-- Export Excel -->
            <a href="{{ route('sales.export.weekly') }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
    ...
    <div class="overflow-x-auto" id="printableArea">
        ...
        <div class="print-only" style="display: none;">
            <div class="text-center mb-6">
                ...
                <p class="text-gray-600">Weekly Sales Report</p>
                <p class="text-gray-600">Week {{ now()->weekOfYear }} - {{ now()->startOfWeek()->format('M d') }} to {{ now()->endOfWeek()->format('M d, Y') }}</p>
                <hr class="my-4">
            </div>
        </div>
        ...
        <div class="print-only" style="display: none;">
            ...
            <div class="text-sm text-gray-600">
                ...
                <p>Week {{ now()->weekOfYear }} - {{ now()->startOfWeek()->format('M d') }} to {{ now()->endOfWeek()->format('M d, Y') }}</p>
...
<style>
    @media print {
        ...
        .bg-gray-50, .bg-blue-50, .hover\:bg-gray-50 {
...
<script>
    ...
    function shareWhatsApp() {
        ...
        const weekStart = "{{ now()->startOfWeek()->format('M d') }}";
        const weekEnd = "{{ now()->endOfWeek()->format('M d, Y') }}";
        
        const message = `📊 *Weekly Sales Report* 📊\n\n` +
                       `🏢 ${businessName}\n` +
                       `📅 Week {{ now()->weekOfYear }}: ${weekStart} - ${weekEnd}\n\n` +
    ...
    function shareEmail() {
        ...
        const subject = `Weekly Sales Report - Week {{ now()->weekOfYear }}`;
        const body = `Weekly Sales Report\n\n` +
